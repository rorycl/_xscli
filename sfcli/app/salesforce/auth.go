...
// It uses a saved token and will fail if the token does not exist.
func NewClient(ctx context.Context, cfg *oauth2.Config, tokenPath string) (*Client, error) {
	...
	tokenSource := cfg.TokenSource(ctx, cache.Token)
	...
	return &Client{
		...
		apiVersion:  "v59.0", // A recent, stable API version
	}, nil
}
